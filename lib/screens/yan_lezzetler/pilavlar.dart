...
class _PilavlarState extends State<Pilavlar> {
  List<dynamic> pilavlar = []; // JSON'dan veri alacağız
  bool isLoading = true;

  // JSON dosyasını yükleme fonksiyonu
  Future<void> loadJson() async {
    final String response = await rootBundle.rootBundle.loadString('assets/yanlezzetler/pilavlar/pilavlar.json');
    final data = json.decode(response); // JSON verisini decode et
    setState(() {
      pilavlar = data; // JSON verisini listeye at
          ...
          children: [
            ...
            Expanded(
              child: isLoading
                  ...
                  : ListView.builder(
                itemCount: pilavlar.length,
                itemBuilder: (context, index) {
                  var pilav = pilavlar[index];
                  return Card(
                      ...
                      leading: Container(
                        ...
                        decoration: BoxDecoration(
                          image: DecorationImage(
                            image: AssetImage(pilav['image_url']),
