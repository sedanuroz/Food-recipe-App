...
class Makarnalar extends StatefulWidget {
  @override
  _MakarnalarState createState() => _MakarnalarState();
}

class _MakarnalarState extends State<Makarnalar> {
  List<dynamic> makarnalar = []; // JSON'dan veri alacağız
  bool isLoading = true;

  // JSON dosyasını yükleme fonksiyonu
  Future<void> loadJson() async {
    final String response = await rootBundle.rootBundle.loadString('assets/yanlezzetler/makarnalar/makarnalar.json');
    final data = json.decode(response); // JSON verisini decode et
    setState(() {
      makarnalar = data; // JSON verisini listeye at
          ...
          children: [
            ...
            Expanded(
              child: isLoading
                  ...
                  : ListView.builder(
                itemCount: makarnalar.length,
                itemBuilder: (context, index) {
                  var makarna = makarnalar[index];
                  return Card(
                      ...
                      leading: Container(
                        ...
                        decoration: BoxDecoration(
                          image: DecorationImage(
                            image: AssetImage(makarna['image_url']),
