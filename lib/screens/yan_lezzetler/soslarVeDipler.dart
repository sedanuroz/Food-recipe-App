...
class Soslar_ve_Dipler extends StatefulWidget {
  @override
  _Soslar_ve_DiplerState createState() => _Soslar_ve_DiplerState();
}

class _Soslar_ve_DiplerState extends State<Soslar_ve_Dipler> {
  List<dynamic> soslar_ve_dipler = []; // JSON'dan veri alacağız
  bool isLoading = true;

  // JSON dosyasını yükleme fonksiyonu
  Future<void> loadJson() async {
    final String response = await rootBundle.rootBundle.loadString('assets/yanlezzetler/soslarVeDipler/soslarVeDipler.json');
    final data = json.decode(response); // JSON verisini decode et
    setState(() {
      soslar_ve_dipler = data; // JSON verisini listeye at
          ...
          children: [
            ...
            Expanded(
              child: isLoading
                  ...
                  : ListView.builder(
                itemCount: soslar_ve_dipler.length,
                itemBuilder: (context, index) {
                  var sos = soslar_ve_dipler[index];
                  return Card(
                      ...
                      leading: Container(
                        ...
                        decoration: BoxDecoration(
                          image: DecorationImage(
                            image: AssetImage(sos['image_url']),
