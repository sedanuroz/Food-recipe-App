...
class KahvaltiliklerPage extends StatefulWidget {
  @override
  _KahvaltiliklerPageState createState() => _KahvaltiliklerPageState();
}

class _KahvaltiliklerPageState extends State<KahvaltiliklerPage> {
  List<dynamic> kahvaltilikler = []; // JSON'dan veri alacağız
  bool isLoading = true;

  // JSON dosyasını yükleme fonksiyonu
  Future<void> loadJson() async {
    final String response = await rootBundle.rootBundle.loadString('assets/kahvaltilikler/kahvaltilikler.json');
    final data = json.decode(response); // JSON verisini decode et
    setState(() {
      kahvaltilikler = data; // JSON verisini listeye at
          ...
          children: [
            ...
            Expanded(
              child: isLoading
                  ...
                  : ListView.builder(
                itemCount: kahvaltilikler.length,
                itemBuilder: (context, index) {
                  var kahvalti = kahvaltilikler[index];
                  return Card(
                      ...
                      leading: Container(
                        ...
                        decoration: BoxDecoration(
                          image: DecorationImage(
                            image: AssetImage(kahvalti['image_url']),
