...
class AnaYemeklerPage extends StatefulWidget {
  @override
  _AnaYemeklerPageState createState() => _AnaYemeklerPageState();
}

class _AnaYemeklerPageState extends State<AnaYemeklerPage> {
  List<dynamic> anaYemekler = []; // JSON'dan veri alacağız
  bool isLoading = true;

  // JSON dosyasını yükleme fonksiyonu
  Future<void> loadJson() async {
    final String response = await rootBundle.rootBundle.loadString('assets/anaYemekler/anayemekler.json');
    final data = json.decode(response); // JSON verisini decode et
    setState(() {
      anaYemekler = data; // JSON verisini listeye at
          ...
          children: [
            ...
            Expanded(
              child: isLoading
                  ...
                  : ListView.builder(
                itemCount: anaYemekler.length,
                itemBuilder: (context, index) {
                  var anaYemek = anaYemekler[index];
                  return Card(
                      ...
                      leading: Container(
                        ...
                        decoration: BoxDecoration(
                          image: DecorationImage(
                            image: AssetImage(anaYemek['image_url']),
