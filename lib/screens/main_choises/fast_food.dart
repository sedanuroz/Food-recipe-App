...
class _FastFoodPageState extends State<FastFoodPage> {
  List<dynamic> fastFoodItems = []; // JSON'dan veri alacağız
  bool isLoading = true;

  // JSON dosyasını yükleme fonksiyonu
  Future<void> loadJson() async {
    final String response = await rootBundle.rootBundle.loadString('assets/fastfood/fastfood.json');
    final data = json.decode(response); // JSON verisini decode et
    setState(() {
      fastFoodItems = data; // JSON verisini listeye at
            ...
            Expanded(
              child: isLoading
                ...
                itemBuilder: (context, index) {
                  var fastFoodItem = fastFoodItems[index];
                  return Card(
                    ...
                    child: ListTile(
                      leading: Container(
                        ...
                        decoration: BoxDecoration(
                          image: DecorationImage(
                            image: AssetImage(fastFoodItem['image_url']),
                      ...
                      subtitle: Text(
                        "${fastFoodItem['calories'][0]['calorie']}",
