...
class Mezeler extends StatefulWidget {
  @override
  _MezelerState createState() => _MezelerState();
}

class _MezelerState extends State<Mezeler> {
  List<dynamic> mezerler = []; // JSON'dan veri alacağız
  bool isLoading = true;

  // JSON dosyasını yükleme fonksiyonu
  Future<void> loadJson() async {
    final String response = await rootBundle.rootBundle.loadString('assets/baslangicler/mezeler/mezeler.json');
    final data = json.decode(response); // JSON verisini decode et
    setState(() {
      mezerler = data; // JSON verisini listeye at
          ...
          children: [
            ...
            Expanded(
              child: isLoading
                  ...
                  : ListView.builder(
                itemCount: mezerler.length,
                itemBuilder: (context, index) {
                  var meze = mezerler[index];
                  return Card(
                      ...
                      leading: Container(
                        ...
                        decoration: BoxDecoration(
                          image: DecorationImage(
                            image: AssetImage(meze['image_url']),
