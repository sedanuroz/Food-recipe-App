...
class DenizUrunleri extends StatefulWidget {
  @override
  _DenizUrunleriState createState() => _DenizUrunleriState();
}

class _DenizUrunleriState extends State<DenizUrunleri> {
  List<dynamic> denizUrunleri = []; // JSON'dan veri alacağız
  bool isLoading = true;

  // JSON dosyasını yükleme fonksiyonu
  //JSON dosyasını okur ve veriyi decode eder.
  Future<void> loadJson() async {
    final String response = await rootBundle.rootBundle.loadString('assets/baslangicler/denizUrunleri/denızUrunler.json');
    final data = json.decode(response); // JSON verisini decode et
    setState(() {
      denizUrunleri = data; // JSON verisini listeye at
          ...
          children: [
            ...
            Expanded(
              child: isLoading
                  ...
                  : ListView.builder(
                itemCount: denizUrunleri.length,
                itemBuilder: (context, index) {
                  var denizUrunu = denizUrunleri[index];
                  return Card(
                      ...
                      leading: Container(
                        ...
                        decoration: BoxDecoration(
                          image: DecorationImage(
                            image: AssetImage(denizUrunu['image_url']),
