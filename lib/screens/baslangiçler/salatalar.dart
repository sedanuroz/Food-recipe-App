...
class _SalatalarState extends State<Salatalar> {
  List<dynamic> salatalar = []; // JSON'dan veri alacağız
  bool isLoading = true;

  // JSON dosyasını yükleme fonksiyonu
  Future<void> loadJson() async {
    final String response = await rootBundle.rootBundle.loadString('assets/baslangicler/salatalar/salatalar.json');
    final data = json.decode(response); // JSON verisini decode et
    setState(() {
      salatalar = data; // JSON verisini listeye at
          ...
          children: [
            ...
            Expanded(
              child: isLoading
                  ...
                  : ListView.builder(
                itemCount: salatalar.length,
                itemBuilder: (context, index) {
                  var salata = salatalar[index];
                  return Card(
                      ...
                      leading: Container(
                        ...
                        decoration: BoxDecoration(
                          image: DecorationImage(
                            image: AssetImage(salata['image_url']),
